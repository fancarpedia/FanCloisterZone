...
class GameServer {
  ...
  handleEngineMessage (ws, { id, type, payload, player, seq }) {
    const randomChanging = ['COMMIT', 'FLOCK_EXPAND_OR_SCORE', 'TILE_CONFIRM', 'SHEPHERD_CONFIRM' ].includes(type) || (type === 'DEPLOY_MEEPLE' && payload.pointer.feature === 'FlyingMachine')
    if (randomChanging) {
      payload = {
        ...payload,
        random: Math.random()
